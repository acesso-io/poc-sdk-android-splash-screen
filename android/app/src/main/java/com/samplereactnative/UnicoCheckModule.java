...
public class UnicoCheckModule extends ReactContextBaseJavaModule implements AcessoBioListener, iAcessoBioSelfie, iAcessoBioDocument {
    ...
    private void callSmartCamera() {
        this.openCamera(CameraMode.SMART, "services.json");
    }

    @RequiresApi(api = Build.VERSION_CODES.M)
    @ReactMethod
    private void callDefaultCamera() {
        this.openCamera(CameraMode.DEFAULT, "services.json");
    }

    @RequiresApi(api = Build.VERSION_CODES.M)
    @ReactMethod
    private void callLivenessCamera() {
        this.openCamera(CameraMode.LIVENESS, "services_ln.json");
    }

    @RequiresApi(api = Build.VERSION_CODES.M)
    @ReactMethod
    private void callDocumentCamera() {
        this.openCamera(CameraMode.DOCUMENT, "services.json");
    }
    ...
    @RequiresApi(api = Build.VERSION_CODES.M)
    private void openCamera(CameraMode mode, String credentialsPath) {
        if (hasPermission()) {
            ...
            getCurrentActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (mode == CameraMode.SMART) {
                        build(true);
                        unicoCheckCamera.prepareSelfieCamera(credentialsPath, new SelfieCameraListener() {
                        ...
                        */
                        ...
                        build(false);
                        unicoCheckCamera.prepareDocumentCamera("services.json", new DocumentCameraListener() {
                            @Override
    ...
    public void onUserClosedCameraManually() {
        ...
        System.out.println("Camera fechada manualmente");
    }
    ...
    public void onSuccessDocument(ResultCamera result) {
        sendEvent(reactContext, "onSuccess", result.getBase64());
    }
